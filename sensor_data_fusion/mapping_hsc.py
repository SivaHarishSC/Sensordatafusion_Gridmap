...
class Occupancy_Gridmap_Laser(Node):
    ...
    def __init__(self, size, resolution, topic, z_max=10, alpha=1, \
        ...
        # qos_profile = QoSProfile(reliability=ReliabilityPolicy....,history=HistoryPolicy....,depth=1) # TODO: set the quality of service
        qos_profile = QoSProfile(reliability=ReliabilityPolicy.RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT,
                                 history=HistoryPolicy.RMW_QOS_POLICY_HISTORY_KEEP_LAST,depth=1) # TODO: set the quality of service
    ...
    def inverse_range_sensor_model(self, x_i, x_k, z_k):
        ...
        if((r > self.z_max) or (r > (z_k_beam+(self.alpha/2))) or ((np.abs(phi - phi_k)) > (self.beta/2))):
            return(self.gridmap.l_0)

        # Occupied cell, range inside of cell   
        # TODO: implement this funtion, which logOdd value shall be returned?
        elif((z_k_beam < self.z_max) and (np.abs(r - z_k_beam) < (self.alpha/2))):
    ...
    def in_perceptual_field(self, x_i, x_k, z_k):
        ...
        phi = np.arctan2(x_i[1]-x_k[1], x_i[0]-x_k[0]) - (x_k[2] % (2. * np.pi)) # Hint: theta/lidar_yaw should be in range of 0..2pi
    ...
    def callbackLaserScan(self, scan):
        ...
        pose[0] += 160 * 0.02 / 3.   # x         # center of the gridmap  
        pose[1] += 160* 0.02 / 2.   # y
        pose[2]  = 0.0                  # theta=sensor_yaw in rad
        quat = euler2quat(0, 0, 0, 'sxyz')  # This gives us quat=[w,x,y,z]. Pay attention to the sequence of quaternion elements here.
        # put the center of the map for the pose
        # in euler2quat put the x_rotation,  y_rotation, z_rotation
        # it will give the letter of name on different sides of the map based on the angle values we give
...
def mapping_with_laser_scan(args = None):
    ...
    my_map = Occupancy_Gridmap_Laser(180, 0.02, "hsc_map")
